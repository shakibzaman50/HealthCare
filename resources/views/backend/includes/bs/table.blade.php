<div class="card-header align-items-center pt-5 gap-2 gap-md-5">
    ...
    <div class="card-toolbar">
        @isset($add)
            <a class="btn btn-primary" id="add_button">Add BS</a>
        @endisset
    </div>
</div>
<div class="card-body pt-0">
    <table class="table align-middle table-row-dashed fs-6 gy-5" id="kt_ecommerce_category_table">
        ...
        <tr class="text-start text-gray-400 fw-bold fs-7 text-uppercase gs-0">
            ...
            <th class="min-w-100px ps-5">Date</th>
            <th class="min-w-50px text-center">Time</th>
            <th class="min-w-50px text-center">Value</th>
            <th class="min-w-50px text-center">Status</th>
            <th class="min-w-50px text-center">Type</th>
            <th class="text-end min-w-70px pe-5">Actions</th>
        </tr>
        </thead>
        <tbody class="fw-semibold text-gray-600">
        @foreach($bloodSugars as $item)
            <tr>
                ...
                <td class="text-center">
                    <div class="text-info fs-6 fw-bold">{{ $item->formattedTime }}</div>
                </td>
                <td class="text-center">
                    <div class="text-info fs-6">{{ $item->level }}</div>
                </td>
                <td class="text-center">
                    <div class="text-info fs-6">{{ $item->status }}</div>
                </td>
                <td class="text-center">
                    <div class="text-info fs-6">{{ $item->sugarType->name }}</div>
                </td>
                <td class="text-end">
