        ...
        <div id="kt_app_toolbar_container" class="app-container d-flex flex-stack ">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 ">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item"><span class="bullet bg-gray-400 w-5px h-2px"></span></li>
                    <li class="breadcrumb-item text-muted">Sugar Unit</li>
                </ul>
            </div>
        </div>
    </div>
                                ...
                                <td class="text-end">
                                    ...
                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4" data-kt-menu="true">
                                        <div class="menu-item px-3">
                                            ...
                                            <a href="{{ route('sugar-units.edit', encrypt($item->id)) }}" class="status-change menu-link px-3 bg-light-{{ $class }} text-{{ $class }}">
                                                <i class="fa fa-eye{{ $item->status==config('basic.status.active') ? '-slash' : '' }} text-{{ $class }} me-2"></i><span id="status-change">{{ $item->status==config('basic.status.active') ? 'Inactive' : 'Active' }}</span>
                                            </a>
                                        </div>
                                        <div class="menu-item px-3 mb-1">
                                            <a data-item="{{ $item }}" class="edit-item menu-link px-3 bg-light-primary text-primary"><i class="fa fa-edit me-2 text-primary"></i>Edit</a>
                                        </div>
                                        <div class="menu-item px-3">
                                            <a id="{{ $item->id }}" class="delete-item menu-link px-3 bg-light-danger text-danger">
    ...
    <div class="modal fade" id="kt_modal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered mw-650px">
            <div class="modal-content">
                <form class="form" action="{{ route('sugar-units.store') }}" method="POST" id="modal_form" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header" id="kt_modal_new_address_header">
                        <h3 class="text-center mx-auto" id="modal_title">Add New Sugar Unit</h3>
                        <div class="btn btn-sm btn-icon btn-active-color-primary" id="closeModalBtn" data-bs-dismiss="modal">
                        ...
                        <div class="scroll-y me-n7 pe-7" id="kt_modal_new_address_scroll"
                            ...
                            <div class="row">
                                <div class="col-12 fv-row mb-5">
                                    ...
                                    <input id="name" value="{{ old('name') }}" type="text" class="form-control form-control-solid" placeholder="sugar unit name" name="name"/>
    ...
    <script>
        $(document).ready(function(){
            ...
            function pushContent() {
                update.val('update');
                btn.text('Update');
                title.text('Edit this Sugar Unit');
            ...
            @if($errors->any())
                ...
                if ("{{ old('update') }}"==='update') {
                    form.attr('action',`{{ route('sugar-units.update','') }}/${localStorage.getItem('id')}`);
                    pushContent();
                }
            @endif
            ...
            $('#add_button').on('click', function () {
                update.val('');
                btn.text('Submit');
                title.text('Add New Sugar Unit');
                method.html('<input type="hidden" name="_method" value="POST">');
                form.attr('action',"{{ route('sugar-units.store') }}");
                error.text('');
                modal.modal('show');
                form[0].reset();
            });
            ...
            $('.edit-item').on('click', function () {
                ...
                $('#name').val(item.name);
                form.attr('action',`{{ route('sugar-units.update','') }}/${item.id}`);
                pushContent();
                error.text('');
                modal.modal('show');
            ...
            $('.delete-item').on('click', function () {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $('#delete_form').attr('action',`{{ route('sugar-units.destroy','') }}/${$(this).attr('id')}`).submit();
                    }
                });
            });
