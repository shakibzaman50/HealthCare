        ...
        <div id="kt_app_toolbar_container" class="app-container d-flex flex-stack ">
            <div class="page-title d-flex flex-column justify-content-center flex-wrap me-3 ">
                ...
                <ul class="breadcrumb breadcrumb-separatorless fw-semibold fs-7 my-0 pt-1">
                    ...
                    <li class="breadcrumb-item"><span class="bullet bg-gray-400 w-5px h-2px"></span></li>
                    <li class="breadcrumb-item text-muted">Weight Feeling List</li>
                </ul>
            </div>
        </div>
    </div>
            ...
            <div class="card card-flush">
                <div class="card-header align-items-center pt-5 gap-2 gap-md-5">
                    ...
                    <div class="card-toolbar">
                        <a class="btn btn-primary" id="add_button">Add Feeling</a>
                    </div>
                </div>
                                ...
                                <td class="text-end">
                                    ...
                                    <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-125px py-4"
                                        ...
                                        <div class="menu-item px-3">
                                            ...
                                            <a href="{{ route('feeling-lists.edit', encrypt($item->id)) }}"
    ...
    <div class="modal fade" id="kt_modal" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered mw-650px">
            <div class="modal-content">
                <form class="form" action="{{ route('feeling-lists.store') }}" method="POST" id="modal_form"
                      enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header" id="kt_modal_new_address_header">
                        <h3 class="text-center mx-auto" id="modal_title">Add New Feeling Unit</h3>
    ...
    <script>
        $(document).ready(function() {
            ...
            if ("{{ old('update') }}" === 'update') {
                form.attr('action', `{{ route('feeling-lists.update','') }}/${localStorage.getItem('id')}`);
                pushContent();
            }
            @endif
            ...
            $('#add_button').on('click', function() {
                ...
                form.attr('action', "{{ route('feeling-lists.store') }}");
                error.text('');
                modal.modal('show');
                form[0].reset();
            });
            ...
            $('.edit-item').on('click', function() {
                ...
                $('#name').val(item.name);
                form.attr('action', `{{ route('feeling-lists.update','') }}/${item.id}`);
                pushContent();
                error.text('');
                modal.modal('show');
            ...
            $('.delete-item').on('click', function() {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $('#delete_form').attr('action', `{{ route('feeling-lists.destroy','') }}/${$(this).attr('id')}`).submit();
                    }
                });
            });
